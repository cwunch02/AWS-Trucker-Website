...
function SponsorLogs() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const pointChangeResult = await fetch(`https://team27-express.cpsc4911.com/point_change?SPONSOR_ID=${sponsorData.SPONSOR_ID}`);
                const pointChangeJsonResult = await pointChangeResult.json();
                setPointLogs(pointChangeJsonResult);
    
                // get application change log
                const appChangeResult = await fetch(`https://team27-express.cpsc4911.com/application_change?SPONSOR_ID=${sponsorData.SPONSOR_ID}`);
                const appChangeJsonResult = await appChangeResult.json();
                setAppLogs(appChangeJsonResult);
    
                // get password change log
                const passChangeResult = await fetch(`https://team27-express.cpsc4911.com/password_change?SPONSOR_ID=${sponsorData.SPONSOR_ID}`);
                const passChangeJsonResult = await passChangeResult.json();
                setPassLogs(passChangeJsonResult);
    
                // get login attempt log
                const logAttemptResult = await fetch(`https://team27-express.cpsc4911.com/login_attempt?SPONSOR_ID=${sponsorData.SPONSOR_ID}`);
    ...
    const filteredTableData = useMemo(() => {
        ...
        return filteredData;
    }, [currentTable, pointLogs, appLogs, passLogs, loginLogs, selectedDateRange]);
